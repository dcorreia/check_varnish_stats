...
sub run_varnishstat_command {
	...
	$command = "$options{'bin'} -1 -f $fields";
	print $command."\n";
	$result = qx($command);
	@ans = split (/\n/s, $result);
	...
	foreach $line (@ans) {
		...
		#print $resultname.': '.$results{$resultname}."\n";
	}
}
...
if($options{'cache'}) {
	check_warn_crit();
	$stats = "MAIN.cache_hit,MAIN.cache_hitpass,MAIN.cache_miss";
...
if($options{'backend'}) {
	...
	if($options{'backend'} eq "all") {
		$stats = "MAIN.backend_conn,MAIN.backend_unhealthy,MAIN.backend_busy,MAIN.backend_fail,MAIN.backend_resuse,MAIN.backend_toolate,MAIN.backend_recycle,MAIN.backend_retry";
		run_varnishstat_command();
		$result="";
		$perfdata="";
		...
		check_warn_crit();	
		$stats = "MAIN.backend_".$options{'backend'};
		run_varnishstat_command();
